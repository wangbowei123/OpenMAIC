...
const COURSE_PRESET_STORAGE_KEY = 'coursePresetId';
...
const initialFormState: FormState = {
  pdfFile: null,
  requirement: DEFAULT_COURSE_PRESET.defaultRequirement,
  webSearch: false,
...
function HomePage() {
  ...
  >(undefined);
  const [selectedCourseId, setSelectedCourseId] = useState<CoursePresetId>(DEFAULT_COURSE_PRESET.id);
  ...
  useEffect(() => {
    ...
    try {
      const savedCourseId = localStorage.getItem(COURSE_PRESET_STORAGE_KEY);
      setSelectedCourseId(getCoursePreset(savedCourseId).id);
    } catch {
      /* localStorage unavailable */
    }
  }, []);
  ...
  const applyCoursePreset = (preset: CoursePreset) => {
    setSelectedCourseId(preset.id);
    updateForm('requirement', preset.defaultRequirement);
    try {
      localStorage.setItem(COURSE_PRESET_STORAGE_KEY, preset.id);
    } catch {
      /* ignore */
    }
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center p-4 pt-16 md:p-8 md:pt-16 overflow-x-hidden">
      ...
      >
        ...
        <Link
          href="/admin"
          className="flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[12px] font-medium text-sky-600 dark:text-sky-300 hover:bg-white dark:hover:bg-gray-700 hover:text-sky-800 dark:hover:text-sky-100 hover:shadow-sm transition-all"
          title="管理员数据看板"
        >
      ...
      <motion.div
        ...
        className={cn(
          'relative z-20 w-full max-w-[980px] flex flex-col items-center',
      ...
      >
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-sky-200/70 bg-white/65 px-3 py-1 text-[12px] font-medium text-sky-700 shadow-sm dark:border-sky-700/40 dark:bg-slate-900/65 dark:text-sky-300">
            <Sparkles className="size-3.5" />
            《离散数学》智能教学资源建设
          </div>
          <h1 className="mt-4 text-2xl md:text-4xl font-semibold tracking-normal text-slate-900 dark:text-slate-50">
            离散数学 AI 互动课程平台
          </h1>
        </motion.div>
      ...
      <div className="mt-auto pt-12 pb-4 text-center text-xs text-muted-foreground/40">
        以《离散数学》为核心的计算机类课程智能教学资源建设项目
      </div>
    </div>
  );
}

function CoursePresetPicker({
  presets,
  selectedId,
  onSelect,
}: {
  presets: CoursePreset[];
  selectedId: CoursePresetId;
  onSelect: (preset: CoursePreset) => void;
}) {
  const corePreset = presets[0];
  const pilotPresets = presets.slice(1);
  const coreSelected = selectedId === corePreset.id;
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={() => onSelect(corePreset)}
        className={cn(
          'group w-full text-left rounded-2xl border bg-white/78 dark:bg-slate-900/78 backdrop-blur-sm p-5 shadow-sm transition-all',
          'hover:-translate-y-0.5 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400',
          coreSelected
            ? 'border-sky-400 ring-2 ring-sky-200/80 dark:ring-sky-800/60'
            : 'border-border/60 hover:border-sky-300/80',
        )}
      >
        <div className="flex flex-col gap-4 md:flex-row md:items-start">
          <span className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-sky-700 dark:bg-sky-900/35 dark:text-sky-300">
            <BookOpen className="size-5" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="flex flex-wrap items-center gap-2">
              <span className="text-lg font-semibold text-foreground">核心课程：{corePreset.title}</span>
            ...
            <p className="mt-3 text-[13px] leading-relaxed text-muted-foreground/85">
              {corePreset.description}
            </p>
            <span className="mt-4 flex flex-wrap gap-1.5">
              {corePreset.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-sky-50 px-2 py-0.5 text-[11px] text-sky-700 dark:bg-sky-950/50 dark:text-sky-300"
                >
                  {tag}
                </span>
              ))}
      ...
      <div className="mt-3 rounded-2xl border border-border/60 bg-white/60 p-4 shadow-sm backdrop-blur-sm dark:bg-slate-900/60">
        <div className="mb-3 flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
              <GitBranch className="size-4 text-emerald-600 dark:text-emerald-300" />
              多课程应用场景
            </div>
            <p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">
              作为《离散数学》智能教学资源建设流程的迁移试点，用于展示多课程推广应用。
            </p>
          </div>
          <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-medium text-emerald-700 dark:bg-emerald-900/35 dark:text-emerald-300">
            试点课程 {pilotPresets.length} 门
          </span>
        </div>

        <div className="grid gap-2 md:grid-cols-2">
          {pilotPresets.map((preset) => {
            const selected = preset.id === selectedId;
            const Icon = preset.id === 'algorithm-design' ? Network : BookOpen;
            ...
            return (
              <button
                ...
                className={cn(
                  'group text-left rounded-xl border bg-white/75 dark:bg-slate-950/40 p-3 transition-all',
                  'hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400',
                  selected
                    ? 'border-emerald-400 ring-2 ring-emerald-200/70 dark:ring-emerald-800/60'
                    : 'border-border/50 hover:border-emerald-300/80',
                )}
              >
                <div className="flex items-start gap-2.5">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-900/35 dark:text-emerald-300">
